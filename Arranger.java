...
public class Arranger {
	...
	private static double[][] readFile(String arrangerFileName)
		...
		double[][] matrix = new double[strToken.countTokens()][strToken.countTokens()];
		int i,j;
		for(i=0;i<strToken.countTokens();i++)
		{
			StringTokenizer strToken_new = new StringTokenizer(str,",");
			for(j=0;j<strToken.countTokens();j++)
			{
				double temp = Double.parseDouble(strToken_new.nextToken());
				matrix[i][j] = temp;
			}
			str = reader.readLine();
		}
	...
	private void saveMatrix(String fileName) throws IOException {
		...
		String str = "";
		int length = arrangerMatrix.length;
		int i=0,j=0;
		for(i=0;i<length;i++)
		{
			for(j=0;j<length;j++)
			{
				str += arrangerMatrix[i][j];
				if(j != length-1)
				{
					str += ",";
				}
