...
public class TestCustomer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			fail("1: Interface Customer not found " + "(check the name of the interface)");
		}

		assertTrue("2: Customer is not an Interface", interfaceDescriptor.isInterface());
		...
		assertTrue("4: Customer should contain one methods", methods.length == 1);

		try {
			Method method = interfaceDescriptor.getMethod("getCustomerId", new Class[0]);
		} catch (NoSuchMethodException nsme) {
			fail("5: Method getCustomerId() not found " + "(check the method signature)");
		}
